   ...
   <h2 class="text-5xl mt-10 text-center text-white m-5 ">Редактирование вопроса</h2>
    <div class="h-30 bg-slate-600 w-100  w-200 shadow-lg max-w-sm mx-auto flex items-center space-x-4 p-5">
        <form action="{{route('task.update', $task->id)}}" method="post" class="w-full">
            @csrf
            <input type="text" placeholder="Название" name="title" class=" {{$errors->has('title') ? 'placeholder-red-400' : 'placeholder-slate-500'}}"
             value="{{$task->title}}">
            @error('title')
                <label class="text-red-400 text-sm">{{$message}}</label>
            @enderror
            <input type="text" placeholder="Описание" name="desc" class=" {{$errors->has('desc') ? 'placeholder-red-400' : 'placeholder-slate-500'}}"
                   value="{{$task->desc}}">
            @error('desc')
                <label class="text-red-400 text-sm">{{$message}}</label>
            @enderror
